...
class Position:
    ...
    def __format__(self, format_spec):  # f_s = ''
...
def auto_repr(cls):
    ...
    signature = inspect.signature(cls.__init__)  # zwraca parametry funkcji w tuple
    parameter_names = list(signature.parameters)[1:]  # zwraca parametry bez selfa, w liście
    print(parameter_names)

    # spr. czy jest wartość parametru, która jest potrzebna do zrobienia __repr__
    if not all(
        isinstance(members.get(name, None), property)
        for name in parameter_names
     ):
        raise TypeError(f'Cannot apply auto_repr to {cls.__name__} because not all __init__ parameters have matching properties')
    ...
    setattr(cls, '__repr__', synthesized_repr)  # settuje atrybuty i wartość
